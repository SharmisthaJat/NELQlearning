...
class GreedyEpsilonPolicy(Policy):
    ...
    def select_action(self, q_values, **kwargs):
        ...
        Parameters
        ----------
        q_values: Function that returns array-like
          lambda: Array-like structure of floats representing the Q-values for
          each action.
        ...
        if random.random() < self.epsilon:
          ...
          policy = GreedyPolicy(self.num_actions)
...
class LinearDecayGreedyEpsilonPolicy(Policy):
    ...
    def __init__(self, num_actions, start_value, end_value,
        ...
        self.increment = (self.end_value - self.start_value) / num_steps
        self.reset()
    ...
    def select_action(self, q_values, **kwargs):
        ...
        Parameters
        ----------
        q_values: lambda: np.array
          A function returning Q-values for each action.
        ...
        Any:
          Selected action.
        """
        eps = np.clip(self.current_value, min(self.start_value, self.end_value), max(self.start_value, self.end_value))
        self.current_value += self.increment

        return GreedyEpsilonPolicy(self.num_actions, eps).select_action(q_values=q_values)
