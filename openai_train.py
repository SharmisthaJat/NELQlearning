...
from openai_agent import RLAgent
...
def train(agent, env, optimizer):
  ...
  for training_steps in range(max_steps):
    ...
    action, reward, is_done = agent.step(epsilon)
    s2 = agent.get_state()
    #print('s2 :', s2)
    tr_reward += reward
    #painter_tr.draw()
    #print(reward)
    if add_to_replay:
      replay.append((s1, action, reward, s2))  # enum issue in server machine
...
def main():
  ...
  optimizer = optim.Adam(agent.policy.parameters(), lr=0.0001)
